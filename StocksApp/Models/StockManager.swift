...
struct StockManager {
    let stockURL = "https://finnhub.io/api/v1/quote?"
    let apiKey = "c10hqqf48v6ujh08p12g"
    
    static let instance = StockManager()
    
    func fetchStock(with ticker: String, completion: @escaping (FHQuote?, Error?) -> Void) {
        let urlString = "\(stockURL)symbol=\(ticker)&token=\(apiKey)"
